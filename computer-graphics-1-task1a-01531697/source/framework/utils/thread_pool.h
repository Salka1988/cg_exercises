...
template <typename F>
void renderthread(F& f, int width, int height, int tile_width, int tile_height, std::atomic<int>& next)
{
...
void launchTiled(F&& f, int num_threads, int width, int height, int tile_width, int tile_height)
{
	...
	for (int i = 0; i < num_threads; ++i)
		pool.launch(std::ref(renderthread<F>), std::ref(f), width, height, tile_width, tile_height, std::ref(next));
}
